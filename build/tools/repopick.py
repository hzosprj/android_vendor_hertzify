...
def is_hertzify_gerrit(remote_url):
    p = urllib.parse.urlparse(remote_url)
    return p.hostname == "review.hertzifyos.org"
...
def main():
    parser = argparse.ArgumentParser(
        ...
        patches from HertzifyOS's Gerrit instance (or any gerrit instance of your choosing)
    ...
    parser.add_argument(
        "-g",
        "--gerrit",
        default="https://review.hertzifyos.org",
        metavar="",
...
def do_git_fetch_pull(args, item):
    ...
    # Try fetching from GitHub first if using hertzify gerrit
    if is_hertzify_gerrit(args.gerrit):
        if args.verbose:
    ...
    # If not using the hertzify gerrit or github failed, fetch from gerrit.
    if args.verbose:
        if is_hertzify_gerrit(args.gerrit):
            print(
